...
.vs-select__label--label {
  transform: translate(-6%, -28px) !important;
}
...
.tabs-item.active a {
  border-bottom: 3px solid #89253e;
  opacity: 1;
  -webkit-animation: tabs-ripple 0.2s linear forwards;
  animation: tabs-ripple 0.2s linear forwards;
}
...
.uk-tab {
  ...
  border-radius: 4px;
  /* border-bottom: 1px solid #89253e !important; */
}
...
.VueCarousel-navigation-button {
  ...
  color: rgba(137, 37, 62, 0.7) !important;
}
...
.select-item--checkbox:last-child {
  margin-right: 0;
}
...
.chromakey__backgrounds {
  width: 100%;
}

.chromakey__polaroid-back {
  position: relative;
  display: block;
...
.chromakey__polaroid-img {
  position: relative;
  font-size: 0;
}

.chromakey__polaroid-img img {
  font-size: 0;
  display: block;
}

.chromakey__polaroid-human {
  display: block;
  content: "";
  position: absolute;
  bottom: 0;
...
.vue-select-image__item:last-child {
...
.chromakey__polaroid-img-wrapper {
  position: absolute;
  top: 0;
  bottom: 0;
  left: 0;
